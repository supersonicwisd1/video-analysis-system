...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results, isLoading }) => {
  ...
  // Get sources from either results or sources array
  const sources = results.sources || results.results || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center text-sm text-gray-500">
        <p>
          Searched for: <span className="font-medium">"{results.query}"</span>
          {results.video_id && (
            <> in video <code className="bg-gray-100 px-2 py-1 rounded font-mono">
              {results.video_id}
            </code></>
          )}
        </p>
